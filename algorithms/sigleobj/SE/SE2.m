function [best_fvalue,best_solution,run_series,run_info,user_set] = SE2(func_name , algo_config,func_config,user_config)
...
%                     newswarm(:,i) = ((1-2*r5).*swarm(:,i) + 2*r5.*((r3.*localBestSolution(:,1) + r4.*globalBestSolution(:,1))));
                    newswarm(:,i) = ((1-3*r5).*swarm(:,i) + 3*r5.*((r3.*swarm(:,randi(PopuSize)) + r4.*globalBestSolution(:,1))));
