    ...
    while FES < maxFES
        ...
        for i = 1:PopuSize
            if(f_swarm(i) >= f_swarm_new(i))
               ...
               Salpha = [Salpha alpha(i)];
            end
        end
        if size(Salpha,2) > 0
            malpha(k) = mean(Salpha);
            k = mod(k+1,H) + 1;
        end
...
%         objfunc.draw_running(swarm);
       ...
       %% ��ɢ����
            for i = 1:PopuSize
                ...
                alpha(i) = malpha(randi(H)) + (0.1*randn(1,1));
                r1 = rand();
                r2 = 1 + r1;
                r3 = rand(DIM,1);
                r4 = 1 - r3;
                r5 = rand(DIM,1);
                newswarm(:,i) = -alpha(i)*r5.*swarm(:,i) + (1 + alpha(i)*r5).*((r3.*swarm(:,randi(PopuSize)) + r4.*globalBestSolution(:,1)));
