...
%         objfunc.draw_running(swarm,'PSO');
       ...
       %% ��ɢ����
            for i = 1:PopuSize
                ...
                r3 = rand(DIM,1);
                r4 = 1 - r3;
                r5 = rm(:,i) + 0.1*randn(DIM,1);
                rw(:,i) = r5;
                    newswarm(:,i) = -r5.*swarm(:,i) + (1+r5).*((r3.*swarm(:,randi(PopuSize)) + r4.*swarm(:,randi(PopuSize))));
...
%                     newswarm(:,i) = ((1-2*r5).*swarm(:,i) + 2*r5.*((r3.*swarm(:,randi(PopuSize)) + r4.*globalBestSolution(:,1))));
            ...
            for i = 1:PopuSize
                r = rand(DIM,1) > p;
                newswarm(:,i) = r.*newswarm(:,i) +(1-r).*swarm(:,i);
            end 
