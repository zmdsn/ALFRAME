...
static double *recorded_x2 = NULL;//[MAX_OF_RUNS][MAX_FUNCTION_NUMBER][RECORDING_POINTS_NUM][10];
static double *recorded_fx2 = NULL;//[MAX_OF_RUNS][MAX_FUNCTION_NUMBER][RECORDING_POINTS_NUM];
...
double *getx(int number_of_run, int func_number, int dim, int evaluate_number)
{
	...
	if (index_record != -1)
	{
		if (dim == 10)
			...
			return lazy_multi_dim_array4(number_of_run - 1, func_number - 1, index_record, 0,
			&recorded_x2, MAX_OF_RUNS, MAX_FUNCTION_NUMBER, RECORDING_POINTS_NUM, dim);
...
double *getfx(int number_of_run, int func_number, int dim, int evaluate_number)
{
	...
	if (index_record != -1)
	{
		if (dim == 10)
			...
			return lazy_multi_dim_array3(number_of_run - 1, func_number - 1, index_record,
			&recorded_fx2, MAX_OF_RUNS, MAX_FUNCTION_NUMBER, RECORDING_POINTS_NUM);
...
static double *currentBest2=NULL;//[MAX_OF_RUNS][MAX_FUNCTION_NUMBER];
static double *currentBest2x=NULL;//[MAX_OF_RUNS][MAX_FUNCTION_NUMBER][2];
static int *evaluate_counter_2=NULL;//[MAX_OF_RUNS][MAX_FUNCTION_NUMBER];
if(dim==10)
	#define GETBEST(dim,i,j) (currentBest10+(i)*MAX_FUNCTION_NUMBER+(j))
else if(dim==30)
	#define GETBEST(dim,i,j) (currentBest30+(i)*MAX_FUNCTION_NUMBER+(j))
else if(dim==2)
	#define GETBEST(dim,i,j) (currentBest2+(i)*MAX_FUNCTION_NUMBER+(j))

if(dim==10)
	#define GETBESTX(dim,i,j) (currentBest10x+(i)*MAX_FUNCTION_NUMBER+(j))
else if(dim==30)
	#define GETBESTX(dim,i,j) (currentBest30x+(i)*MAX_FUNCTION_NUMBER+(j))
else if(dim==2)
	#define GETBEST(dim,i,j) (currentBest2x+(i)*MAX_FUNCTION_NUMBER+(j))

if(dim==10)
	#define GETEC(dim,run,fn) (evaluate_counter_10+(run-1)*MAX_FUNCTION_NUMBER+fn-1)
else if(dim==30)
	#define GETEC(dim,run,fn) (evaluate_counter_30+(run-1)*MAX_FUNCTION_NUMBER+fn-1)
else if(dim==2)
	#define GETEC(dim,run,fn) (evaluate_counter_2+(run-1)*MAX_FUNCTION_NUMBER+fn-1)
...
void getbest(double** x, double** fx, int dim, int run, int func_num)
{
	...
	if (currentBest2==NULL)
	{
		currentBest2 = (double*)malloc(sizeof(double)*MAX_OF_RUNS*MAX_FUNCTION_NUMBER);
		for (i=0; i<MAX_OF_RUNS*MAX_FUNCTION_NUMBER; i++)
			currentBest2[i] = INF;
	}
	if (currentBest2x ==NULL)
	{
		currentBest2x = (double*)malloc(sizeof(double)*MAX_OF_RUNS*MAX_FUNCTION_NUMBER*2);
	}
...
{
	if (evaluate_counter_2==NULL)
	{
		evaluate_counter_2 = (int*) malloc(sizeof(int)*MAX_OF_RUNS*MAX_FUNCTION_NUMBER);
		memset(evaluate_counter_2, 0 , sizeof(int)*MAX_OF_RUNS*MAX_FUNCTION_NUMBER);
	}
...
static int dims_to_eval[] = { 2, 10, 30 };
