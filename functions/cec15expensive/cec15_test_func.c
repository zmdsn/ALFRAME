...
void cec15_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		if (!(nx==2||nx==10||nx==30))
		{
			printf("\nError: Test functions are only defined for D=2,10,30.\n");
		}
		/* Load Matrix M*/
		sprintf(FileName, "functions/cec15expensive/input_data/M_%d_D%d.txt", func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "functions/cec15expensive/input_data/shift_data_%d_D%d.txt", func_num,nx);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\n Error: Cannot open input file for reading \n");
		...
		/* Load Shuffle_data */
		sprintf(FileName, "functions/cec15expensive/input_data/shuffle_data_%d_D%d.txt", func_num,nx);

		if (func_num>=10&&func_num<=12)
		{
			//sprintf(FileName, "../../../src/functions/cec15expensive/input_data/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
				printf("\n Error: Cannot open input file for reading \n");
