       ...
       %% 构造函数 constructed function
        function obj = f8(specific)
            ...
            % 设置函数参数的默认值
                ...
                if ~isfield(specific,'Xmin') specific.Xmin = -500*ones(specific.Xdim,1); end  % 下界 
                if ~isfield(specific,'Xmax') specific.Xmax = 500*ones(specific.Xdim,1);  end   % 上界
                if ~isfield(specific,'Fmin') specific.Fmin = 0;  end  % 目标函数最小值
                if ~isfield(specific,'Tex')  specific.Tex  =  '\sum\limits_{i=1}^D(x_i^2 -10cos(2\pi x_i) +10)';   end   % 函数tex表达式
