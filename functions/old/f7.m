       ...
       %% 构造函数 constructed function
        function obj = f7(specific)
            ...
            % 设置函数参数的默认值
                ...
                if ~isfield(specific,'Xmin') specific.Xmin = -1.28*ones(specific.Xdim,1); end  % 下界 
                if ~isfield(specific,'Xmax') specific.Xmax = 1.28*ones(specific.Xdim,1);  end   % 上界
                if ~isfield(specific,'Fmin') specific.Fmin = 0;  end  % 目标函数最小值
                if ~isfield(specific,'Tex')  specific.Tex  =  '\sum\limits_{i=1}^Dix_i^4 + random[0,1]';   end   % 函数tex表达式
