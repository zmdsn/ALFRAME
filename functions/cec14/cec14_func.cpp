...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
	...
	n = mxGetM (prhs[0]);
    if (!(n==2||n==10||n==30||n==50||n==100||(func_num==17&&n==3)))
    {
		mexPrintf ("usage: f = cec14_func(x, func_num);\n");
...
void cec14_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "functions/cec14/input_data/M_%d_D%d.txt", func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "functions/cec14/input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\n Error: Cannot open input file for reading \n");
		...
		{
			sprintf(FileName, "functions/cec14/input_data/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
				printf("\n Error: Cannot open input file for reading \n");
