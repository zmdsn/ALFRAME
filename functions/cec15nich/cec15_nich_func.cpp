...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]) 
{
	...
	if ((nrhs < 2) || (nlhs < 1))
    {
		mexPrintf ("usage: f = cec15_nich_func(x, func_num);\n");
		mexErrMsgTxt ("example: f= cec15_nich_func([3.3253000e+000, -1.2835000e+000]', 1);");
    }
    n = (int)mxGetM (prhs[0]);
    m = (int)mxGetN (prhs[0]);
	...
	f = mxGetPr (plhs[0]);
	cec15_nich_func(&x[0], &f[0], n,m,func_num);
}
...
void cec15_nich_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "functions/cec15nich/input_data/M_%d_D%d.txt", func_num,nx);
        // printf("%s",FileName);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
		    mexErrMsgTxt("\n Error: Cannot open input file for reading : input_data \n");
		}        
		...
		/* Load shift_data */
		sprintf(FileName, "functions/cec15nich/input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			mexErrMsgTxt("\n Error: Cannot open input file for reading : shift_data \n");
		}
